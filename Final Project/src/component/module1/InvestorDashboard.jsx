    ...
    <div className="home-links">
      ...
      <div className="profile-container" tabIndex="0">
        ...
        <div className="profile-dropdown">
          ...
          <button onClick={() => setActiveView('password')}>Update Password</button>
          <button onClick={() => setActiveView('terms')}>Terms & Conditions</button>
          <hr />
                      ...
                      {filteredPortfolios.length > 0 ? (
                        filteredPortfolios.map((port, index) => (
                          <React.Fragment key={port.portfolioId}>
                            ...
                            >
                              <td>{index + 1}</td>
                              <td><strong>PF-{port.portfolioId}</strong></td>
                              <td>{port.portfolioName}</td>
                              <td>{port.investedAmount?.toLocaleString()}</td>
                              <td>
                                <span className={`status-badge ${(port.status || 'Pending').toLowerCase()}`}>
                                  {port.status || "Pending"}
                                </span>
                              </td>
                                 <td>{port.quantity || 0} units</td>
